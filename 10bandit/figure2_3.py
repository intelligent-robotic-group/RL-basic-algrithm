...
class Bandit:
    def __init__(self,k_arm=10,epsilon=0.1,initial=0., step_size=0.1, sample_averages=False,true_reward=0.,alfa=0.1):
...
def exercise2_6(runs=2000, time=1000):

    bandits = Bandit(initial=0.,epsilon=0.3)
    best_action_counts, rewards = simulate(runs, time, bandits)
    bandits1 = Bandit(initial=5.0,epsilon=0.1)
    best_action_counts1, rewards1 = simulate(runs, time, bandits1)

    plt.plot(best_action_counts1, label='epsilon = 0, q = 5')
    plt.plot(best_action_counts, label='epsilon = 0.1, q = 0')
    plt.xlabel('Steps')
